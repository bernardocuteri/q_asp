...
public class QAsp {
	...
	public QuantifiedProgram readQuantifiedProgram(String filename) {
		try {
			Program p = readAspProgram(filename);

			System.out.println("Printing of parsed program:");
	...
	private boolean solve(String programFile) {
		...
		for (int i = 0; i < qp.getPrograms().size(); i++) {
			...
			System.out.println("program with choices: \n"+programWithChoiceSB);
		...
		for (int i = qp.getPrograms().size()-2; i >= 0; i--) {
			...
			if(quantifier.equals(EXISTS)) {
				qcirProgram.addGateDefinition(new GateDefinition("psi_"+i, "and", Arrays.asList(current, "psi_"+(i+1))));
			} else if(quantifier.equals(FORALL)) {
				qcirProgram.addGateDefinition(new GateDefinition("psi_"+i, "or", Arrays.asList("-"+current, "psi_"+(i+1))));
			} else {
				throw new IllegalArgumentException("invalid quantifier order (constraint must be the last)");
			}
		}

		System.out.println(qcirProgram.toString());
			...
			for(int i=0;i<clause.size();i++) {
				...
				if(!mappedVars.containsKey(var)) {
					...
					if(atom == null) {
						...
						System.out.println("mapping lit "+atom+" to "+atom2var.get(atom));
					}
				}
	...
	private CNFProgram ground2Sat(LParseProgram ground) {
		...
		System.out.println("ground program transofrming to sat");
